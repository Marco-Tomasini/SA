...
$port     = 8883; 
$client_id = "phpmqtt-mysqli-subscriber-" . rand();
$username = 's1_Brayan';
...
// 3. Tópicos a serem assinados, usando uma função anônima (closure) como callback
// A closure chama a função de inserção simples definida em conexao_simples.php
$topics = [
    "S1/umidade"     => ["qos" => 0, "function" => function ($topic, $msg) {
        $value = trim($msg);
        echo "[" . date("Y-m-d H:i:s") . "] MENSAGEM RECEBIDA: Tópico: $topic, Mensagem: $value\n";
        inserir_dado_simples($topic, $value);
    }],
    "S1/temperatura" => ["qos" => 0, "function" => function ($topic, $msg) {
        $value = trim($msg);
        echo "[" . date("Y-m-d H:i:s") . "] MENSAGEM RECEBIDA: Tópico: $topic, Mensagem: $value\n";
        inserir_dado_simples($topic, $value);
    }],
    "S1/iluminacao"  => ["qos" => 0, "function" => function ($topic, $msg) {
        $value = trim($msg);
        echo "[" . date("Y-m-d H:i:s") . "] MENSAGEM RECEBIDA: Tópico: $topic, Mensagem: $value\n";
        inserir_dado_simples($topic, $value);
    }],
...
echo "Iniciando o loop de escuta contínua... Pressione Ctrl+C para parar.\n";
while(true){
    $mqtt->proc();
